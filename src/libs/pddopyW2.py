    ...
    for pt in range(coordVec.shape[0]):
        for j in range(start_idx[pt],end_idx[pt]):
            ...
            if currentBondStatus == alive:
                ...
                #Calculate Sbar for bond pt_j (pt is main point j in fam member)
                Sbar11 = (1-Damage[j])*0.5*(S11+jS11) * mu[j] * vols[neighbors[j]] #* vols[pt]
                Sbar12 = (1-Damage[j])*0.5*(S12+jS12) * mu[j] * vols[neighbors[j]] #* vols[pt]
                Sbar22 = (1-Damage[j])*0.5*(S22+jS22) * mu[j] * vols[neighbors[j]] #* vols[pt]
